  ...
  return (
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        <button className={`py-1 px-3 border-rounded text-sm transition-all sm:hidden ${showFilter ? 'bg-primary tect-white' :'' }`} onClick={()=>setShowFilter(prev => !prev)}>Filters</button>
        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
        {
          filterDoc.map((item,index)=>(
            <div onClick={()=>navigate(`/appointment/${item._id}`)} className ='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all-500'key={index}>
